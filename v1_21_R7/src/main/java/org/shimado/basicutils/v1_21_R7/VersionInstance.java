...
public class VersionInstance implements IVersionControl {
    ...
    public void createFirework(List<Player> players, Location loc, ItemStack fireworkItem) {
        ...
        Packet spawnPacket = NMSUtil.getPacketPlayOutSpawnEntity(firework, loc);
        players.forEach(p -> NMSUtil.sendPacket(p, spawnPacket));

        firework.i = 0;

        Packet metaPacket = new PacketPlayOutEntityMetadata(NMSUtil.getEntityID(firework), NMSUtil.getDataWatcher(firework).b());
