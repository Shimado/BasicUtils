...
public class VersionInstance implements IVersionControl {
    ...
    public void createFirework(List<Player> players, Location loc, ItemStack fireworkItem) {
        ...
        Packet spawnPacket = new PacketPlayOutSpawnEntity(NMSUtil.getEntityID(firework), firework.cG(), loc.getX(), loc.getY(), loc.getZ(), firework.dM(), firework.dO(), firework.aq(), 1, firework.dz(), firework.cA() * 1.0);
    ...
    public void spawnParticleNote(World world, double x, double y, double z, float color) {
        Packet packet = new PacketPlayOutWorldParticles(Particles.ab, false, x, y, z, color, 0f, 0f, 1, 0);
    ...
    public void spawnParticleDust(World world, double x, double y, double z, float r, float g, float b) {
        ...
        int G = (int) (g * 255);
        int B = (int) (b * 255);
        Packet packet = new PacketPlayOutWorldParticles(new ParticleParamRedstone((R << 16) | (G << 8) | B, 1f), false, x, y, z, 0f, 0f, 0f, 1, 1);
        Bukkit.getOnlinePlayers().forEach(p -> {
            if(world.getUID().equals(p.getWorld().getUID())){
                NMSUtil.sendPacket(p, packet);
            }
    ...
    public Pair spawnArmorStandByLocation(Location loc, org.bukkit.inventory.ItemStack itemHead, boolean isSmall, float angleX, float angleY, float angleZ){
        ...
        Packet packet = new PacketPlayOutSpawnEntity(NMSUtil.getEntityID(stand), stand.cG(), stand.dB(), stand.dD(), stand.dH(), stand.dM(), stand.dO(), stand.aq(), 1, stand.dz(), (double) stand.cA());
    ...
    public void spawnArmorStandToPlayer(Player player, Object standRaw, org.bukkit.inventory.ItemStack itemHead, Object packetRaw){
        ...
        Packet packetSpawn = new PacketPlayOutSpawnEntity(NMSUtil.getEntityID(stand), stand.cG(), stand.dB(), stand.dD(), stand.dH(), stand.dM(), stand.dO(), stand.aq(), 1, stand.dz(), (double) stand.cA());
