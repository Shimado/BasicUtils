package org.shimado.basicutils.v1_21_R1;
...
public class VersionInstance implements IVersionControl {
    ...
    public void createFirework(List<Player> players, Location loc, ItemStack fireworkItem) {
        ...
        Packet spawnPacket = new PacketPlayOutSpawnEntity(NMSUtil.getEntityID(firework), firework.cz(), loc.getX(), loc.getY(), loc.getZ(), firework.dG(), firework.dE(), firework.am(), 1, firework.dr(), firework.ct() * 1.0);
        players.forEach(p -> NMSUtil.sendPacket(p, spawnPacket));

        firework.f = 0;

        Packet metaPacket = new PacketPlayOutEntityMetadata(NMSUtil.getEntityID(firework), firework.ar().b());
    ...
    public void spawnParticleNote(World world, double x, double y, double z, float color) {
        Packet packet = new PacketPlayOutWorldParticles(Particles.aa, false, x, y, z, color, 0f, 0f, 1, 0);
    ...
    public void spawnParticleDust(World world, double x, double y, double z, float r, float g, float b) {
        Packet packet = new PacketPlayOutWorldParticles(new ParticleParamRedstone(new Vector3f(r, g, b), 1f), false, x, y, z, 0f, 0f, 0f, 1, 1);
        Bukkit.getOnlinePlayers().forEach(p -> {
            if(world.getUID().equals(p.getWorld().getUID())){
                NMSUtil.sendPacket(p, packet);
            }
    ...
    public Pair spawnArmorStandByLocation(Location loc, org.bukkit.inventory.ItemStack itemHead, boolean isSmall, float angleX, float angleY, float angleZ){
        ...
        Packet packet = new PacketPlayOutSpawnEntity(NMSUtil.getEntityID(stand), stand.cz(), stand.dt(), stand.dv(), stand.dz(), stand.dG(), stand.dE(), stand.am(), 1, stand.dr(), (double) stand.ct());
        ...
        try {
            ...
            Field invis = Entity.class.getDeclaredField("ap");
    ...
    public void spawnArmorStandToPlayer(Player player, Object standRaw, org.bukkit.inventory.ItemStack itemHead, Object packetRaw){
        ...
        Packet packetSpawn = new PacketPlayOutSpawnEntity(NMSUtil.getEntityID(stand), stand.cz(), stand.dt(), stand.dv(), stand.dz(), stand.dG(), stand.dE(), stand.am(), 1, stand.dr(), (double) stand.ct());
    ...
    public Object createItem(List<Player> players, Location loc, ItemStack itemToDrop, double vectorX, double vectorY, double vectorZ) {
        ...
        Packet spawnPacket = new PacketPlayOutSpawnEntity(NMSUtil.getEntityID(item), item.cz(), loc.getX(), loc.getY(), loc.getZ(), item.dG(), item.dE(), item.am(), 1, item.dr(), item.ct());
